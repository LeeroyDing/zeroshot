...
async function detectContext(cwd = process.cwd()) {
  ...
  try {
    ...
    const branches = execSync('jj log -r "@" --template "branches"', {
      cwd,
      encoding: 'utf8',
    }).trim();
    let branch = branches.split('\n')[0].trim();
    const commitId = execSync('jj log -r "@" --template "commit_id.short()"', {
      cwd,
      encoding: 'utf8',
    }).trim();
    const changeId = execSync('jj log -r "@" --template "change_id.short()"', {
      cwd,
      encoding: 'utf8',
    }).trim();
