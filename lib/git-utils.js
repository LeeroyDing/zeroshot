 ...
 */
function isGitRepo(directory = process.cwd()) {
  try {
    execSync('git rev-parse --git-dir', {
      cwd: directory,
      stdio: 'pipe',
...
/**
 * Get the worktree paths of the git repository.
 * @param {string} [directory=process.cwd()] - Directory within the repository
...
function getWorktreePaths(directory = process.cwd()) {
  try {
    ...
    // The main worktree is not listed by `git worktree list` unless it's a bare repo.
    // So we add the repo root to the list.
    const root = getRepoRoot(directory);
    if (root && !worktrees.includes(root)) {
      worktrees.unshift(root);
    }
    return worktrees;
  } catch {
    const root = getRepoRoot(directory);
