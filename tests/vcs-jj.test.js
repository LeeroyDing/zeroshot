...
describeOrSkip('VCS: jj', () => {
    ...
    it('isRepo returns false for a non-jj repo', async () => {
        execSyncStub.withArgs('jj root', { cwd: tmpDir, stdio: 'pipe' }).throws(new Error('not a repo'));
        const isRepo = await jj.isRepo(tmpDir);
        expect(isRepo).to.be.false;
    });

    it('getRemoteUrl returns the remote URL', async () => {
        execSyncStub.withArgs('jj git remote url origin', { encoding: 'utf8' }).returns('git@github.com:example/repo.git\n');
    ...
    describe('detectContext', () => {
        ...
        it('returns context for a repo with a branch', async () => {
            execSyncStub.withArgs('jj git remote url origin', { cwd: tmpDir, encoding: 'utf8' }).returns('https://github.com/example/repo.git\n');
            execSyncStub.withArgs('jj log -r "@" --template "branches"', { cwd: tmpDir, encoding: 'utf8' }).returns('main\n');
            execSyncStub.withArgs('jj log -r "@" --template "commit_id.short()"', { cwd: tmpDir, encoding: 'utf8' }).returns('1234567\n');
            execSyncStub.withArgs('jj log -r "@" --template "change_id.short()"', { cwd: tmpDir, encoding: 'utf8' }).returns('abcdefg\n');
        ...
        it('returns context for a repo on an anonymous commit', async () => {
            execSyncStub.withArgs('jj git remote url origin', { cwd: tmpDir, encoding: 'utf8' }).returns('https://github.com/example/repo.git\n');
            execSyncStub.withArgs('jj log -r "@" --template "branches"', { cwd: tmpDir, encoding: 'utf8' }).returns('\n');
            execSyncStub.withArgs('jj log -r "@" --template "commit_id.short()"', { cwd: tmpDir, encoding: 'utf8' }).returns('1234567\n');
            execSyncStub.withArgs('jj log -r "@" --template "change_id.short()"', { cwd: tmpDir, encoding: 'utf8' }).returns('abcdefg\n');
        ...
        it('returns null if not a repo', async () => {
            ...
            execSyncStub.withArgs('jj root', { cwd: sinon.match.any, stdio: 'pipe' }).throws(new Error('not a repo'));
            const context = await jj.detectContext(tmpDir);
            expect(context).to.be.null;
        });

        it('returns null if remote command fails', async () => {
            execSyncStub.withArgs('jj git remote url origin', { cwd: tmpDir, encoding: 'utf8' }).throws(new Error('no remote'));
            const context = await jj.detectContext(tmpDir);
            expect(context).to.be.null;
        });
    });
    ...
    describe('worktree functions', () => {
        describe('worktreeAdd', () => {
            it('uses `jj workspace add` when supported', async () => {
                ...
                expect(execSyncStub.calledWith(`jj workspace add ${worktreePath}`, { cwd: tmpDir })).to.be.true;
                expect(execSyncStub.calledWith('jj branch create new-branch', { cwd: worktreePath })).to.be.true;
            });
        ...
        describe('worktreeRemove', () => {
            it('uses `jj workspace forget` when supported', async () => {
                ...
                expect(isWorkspaceSupportedStub.called).to.be.true;
                expect(execSyncStub.calledWith(`jj workspace forget --ignore-working-copy ${worktreePath}`, { cwd: tmpDir })).to.be.true;
            ...
            it('still removes directory if `jj workspace forget` fails', async () => {
                const isWorkspaceSupportedStub = sinon.stub(jj, 'isWorkspaceSupported').resolves(true);
                execSyncStub.withArgs(sinon.match('jj workspace forget')).throws(new Error('not a workspace'));
                const worktreePath = path.join(tmpDir, 'worktree');
                await fs.mkdir(worktreePath);
        ...
        it('getWorktree returns the workspace root', async () => {
            execSyncStub.withArgs('jj workspace root', { cwd: tmpDir, encoding: 'utf8' }).returns(`${tmpDir}\n`);
        ...
        it('remotes returns a list of remotes', async () => {
            ...
            execSyncStub.withArgs('jj git remote list', { cwd: tmpDir, encoding: 'utf8' }).returns(remoteOutput);
        ...
        it('changedFiles returns unstaged and untracked files', async () => {
            ...
            execSyncStub.withArgs('jj status --no-pager -T json', { cwd: tmpDir, encoding: 'utf8' }).returns(JSON.stringify(statusOutput));
        ...
        it('commit creates a commit and returns the commit id', async () => {
            ...
            execSyncStub.withArgs('jj log -r "@" -T json', { cwd: tmpDir, encoding: 'utf8' }).returns(JSON.stringify(logOutput));
