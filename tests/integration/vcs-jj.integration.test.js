...
describeOrSkip('VCS: jj (integration)', () => {
  ...
  it('detects a jj repo correctly', () => {
    expect(jj.isRepo(repoDir)).to.be.true;
    expect(jj.isRepo(os.tmpdir())).to.be.false;
  });

  it('gets the repo root', () => {
    const root = jj.getRoot(repoDir);
  ...
  it('lists remotes', async () => {
    ...
    expect(remotes).to.deep.equal([{ name: 'origin', url: 'https://github.com/example/repo.git' }]);
  });
